...
class TasksIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "should fail creating a new task" do
    get category_path(@user.username, @category)
    assert_response :success
    get new_category_path(@user.username, @category)
    assert_response :success
    assert_no_difference "Task.count" do
      post category_tasks_path(@user.username, @category), params: { task: { name: "", details: "Some details", deadline: DateTime.now + 1} }
    end
  ...
  test "should show and fail updating existing task" do
    get category_path(@user.username, @category)
    assert_response :success
    get category_task_path(@user.username, @category, @task)
    assert_response :success
    ...
    assert_response :success
    assert_no_difference "Task.count" do
      patch category_task_path(@user.username, @category, @task), params: { task: { name: "" } }
    end
    assert_response :unprocessable_entity
  end

  test "should toggle completion of an existing task" do
    get category_path(@user.username, @category)
    assert_response :success
    assert_no_difference "Task.count" do
